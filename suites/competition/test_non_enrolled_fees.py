...
overlay must NEVER apply to account that never enrolled. its fees follow standard schedule
only. perp_maker fixture is non-enrolled counterparty. roles flip vs enrolled-subject tests:
enrolled account RESTS liquidity (one tick inside spread), non-enrolled perp_maker LIFTS as
...
class TestCompetitionNonEnrolledFees:
    def test_non_enrolled_taker_perp_fill_charged_standard_fees_overlay_never_applies(self, account, perp_maker):
        # arrange: enrolled account rests SELL as best ask so non-enrolled maker can lift.
        mkt = resolve_perp_market(account.trading_client, perp_market)
        ...
        create_perp_order(account.order_client, account.app_session_id, market=mkt.market, side="sell",
                          direction="short", type="limit", amount=amount, price=ask_price, tif="gtc",
                          leverage=perp_trade.leverage)

        # act: NON-ENROLLED perp_maker market-buys, lifts enrolled account ask (maker is taker).
        ...
        # assert: no overlay for non-enrolled account. effective == standard.
