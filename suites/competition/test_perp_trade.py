...
rests limit inside spread (guaranteed counterparty), enrolled account (subject) hits it with
...
class TestCompetitionPerpTrade:
    def test_enrolled_taker_perp_fill_charged_resolved_fee_and_counts_toward_volume(self, account, perp_maker):
        ...
        mkt = step("resolve perp market", lambda: resolve_perp_market(account.trading_client, perp_market))
        mark = step("read mark price", lambda: get_perp_mark_price(account.trading_client, mkt.market))
        ...
        volume_before = float(get_fee_tier_effective(account.trading_client).overlay.campaign_volume_usd or "0")
        ...
        def take_and_await():
            order_uuid = create_perp_order(account.order_client, account.app_session_id, market=mkt.market,
                                           side="buy", direction="long", type="market", amount=amount,
                                           leverage=perp_trade.leverage)
            poll_until(
                lambda: get_perp_fills_for_order(account.trading_client, account.app_session_id, mkt.market, order_uuid).fills,
                lambda fills: fills > 0, timeout_s=15, message="perp taker fill appeared",
            )
            return get_perp_fills_for_order(account.trading_client, account.app_session_id, mkt.market, order_uuid)
        ...
        eff_taker = bps_to_rate(get_fee_tier_effective(account.trading_client).effective.perp_taker_bps)
        ...
        volume_after = float(get_fee_tier_effective(account.trading_client).overlay.campaign_volume_usd or "0")
