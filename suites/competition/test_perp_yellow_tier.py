"""YELLOW-balance tier path. port of suites/competition/perp-yellow-tier.spec.ts.

comp tiers use EITHER-threshold rule: tier qualifies by campaign volume OR by 24h-average
YELLOW balance. so holding enough YELLOW deepens overlay tier with NO trading volume.

@trades: funds via uat faucet (deposits YELLOW), places NO orders. no retries so a retry
can't double-deposit. fresh enrolled wallet starts at Base (0 volume). faucet ~24x next tier
YELLOW requirement (campaign_yellow_balance is 24h hour-weighted average, so one deposit
~= deposit/24 crosses it in a tick), then assert either-threshold tier deepened.

DOWN path (sell YELLOW -> tier reverts) omitted on purpose: 24h average cannot drain within
...
@pytest.mark.trades
@pytest.mark.timeout(360)  # enroll + two backend ingestions (overlay activation, then YELLOW 24h average)
class TestCompetitionYellowTier:
    def test_holding_yellow_deepens_overlay_tier_via_either_threshold_rule(self, fresh_wallet, env):
        ...
        # YELLOW requirement (for 0-volume account that's VIP1). no hardcoded thresholds.
        ...
        target = next((t for t in tiers if t.level > vol_tier.level and t.yellow_min > 0), None)
        assert target is not None, "a higher tier with a YELLOW requirement exists to target"
        pre_eff_perp_taker = bps_to_rate(before.effective.perp_taker_bps)

        # act: faucet ~24x target YELLOW requirement so 24h average crosses it, then wait
        # campaign_yellow_balance to ingest up to requirement.
        deposit = math.ceil(target.yellow_min * yellow_flow.deposit_multiplier)
        # faucet client retries 5xx (client_for default) ON PURPOSE: UAT faucet 503s transient,
        # YELLOW credit additive. duplicate deposit harmless (2x deposit still keeps 24h average
        # below next tier requirement). reliability on transient 503 > idempotency here.
        faucet = env.client_for(None, env.faucet_url)
        faucet_deposit(faucet, wallet.app_session_id, str(deposit), yellow_flow.asset)
        poll_until(
            lambda: float(get_fee_tier_effective(trading).overlay.campaign_yellow_balance or "0"),
            lambda y: y >= target.yellow_min, timeout_s=yellow_flow.ingest_timeout_s, intervals=(5,),
            message="campaign_yellow_balance ingested up to the target requirement",
        )

        # assert: either-threshold tier now above volume-only tier, overlay resolved to it,
        # best-of(standard, overlay) stepped effective perp taker DOWN, via YELLOW alone.
        after = get_fee_tier_effective(trading)
        campaign_yellow = float(after.overlay.campaign_yellow_balance or "0")
        either_tier = expected_tier_either(tiers, vol_now, campaign_yellow)
        assert either_tier.level > vol_tier.level, "YELLOW qualifies a tier above the volume-only tier"
        ...
        assert bps_to_rate(after.effective.perp_taker_bps) == pytest.approx(min(standard, overlay), abs=1e-6), \
            "effective == best-of(standard, YELLOW-deepened overlay)"
        assert bps_to_rate(after.effective.perp_taker_bps) < pre_eff_perp_taker, \
            "effective perp taker stepped down via YELLOW"
