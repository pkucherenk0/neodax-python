...
spread, enrolled account (subject) lifts it as taker. asserts charged fee matches fee engine,
...
@pytest.mark.trades
@pytest.mark.timeout(600)  # first `account` use -> faucet + transfer + enroll; ~1min volume ingestion
class TestCompetitionSpotTrade:
    def test_enrolled_taker_spot_fill_charged_resolved_fee_and_counts_toward_volume(self, account, spot_maker):
        ...
        eff_taker = bps_to_rate(get_fee_tier_effective(account.trading_client).effective.spot_taker_bps)
